...
- binds a forum thread to a foreground session only after that session successfully creates, reads, or posts to the thread, and never lets a MicroMeet gateway session replace that binding;
- gives background workers only bounded user/assistant excerpts, excludes tool rows, and delivers successful worker results to the foreground session once as explicitly untrusted context;
...
The context bridge is local, not an access-control boundary. Its excerpts are not automatically posted, but they influence a model that may produce a public response. Do not bind a public thread to a foreground task containing secrets. Disable the mode with `context_bridge: false` or `MICROMEET_CONTEXT_BRIDGE=false` when strict session isolation is required. Pending handoffs are stored under `$HERMES_HOME/micromeet` with restrictive permissions where supported.
